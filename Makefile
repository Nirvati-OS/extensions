...
#
# Generated on 2026-08-04T19:13:24Z by kres undefined.
...
TOOLS_PREFIX ?= ghcr.io/nirvati-os
...
$(TARGETS): $(ARTIFACTS)/bldr
	@$(MAKE) docker-$@ TARGET_ARGS="--tag=$(REGISTRY)/$(USERNAME)/$@:$(shell $(ARTIFACTS)/bldr eval --target $@ --build-arg TAG=$(TAG) '{{.VERSION}}' 2>/dev/null)$(IMAGE_NAME_SUFFIX) --push=$(PUSH) --metadata-file=$(ARTIFACTS)/$@.metadata.json"

.PHONY: push-manifests
push-manifests: $(ARTIFACTS)/bldr  ## Creates and pushes multi-arch manifests combining the per-arch images built by the CI build matrix.
	@for target in $(TARGETS); do \
		tag=$$($(ARTIFACTS)/bldr eval --target $$target --build-arg TAG=$(TAG) '{{.VERSION}}' 2>/dev/null); \
		images=""; \
		for arch in $(ARCHS); do images="$$images $(REGISTRY)/$(USERNAME)/$$target:$$tag-$$arch"; done; \
		docker buildx imagetools create --tag $(REGISTRY)/$(USERNAME)/$$target:$$tag $$images; \
	done
...
internal/extensions/descriptions.yaml: internal/extensions/image-digests
	@if [ "$(REGISTRY)" = "dev-registry.nirvati.de" ]; then \
	  echo "$$NIRVATI_DEV_REGISTRY_PASSWORD" | docker login $(REGISTRY) -u "$$NIRVATI_DEV_REGISTRY_USERNAME" --password-stdin; \
	else \
	  echo "$$NIRVATI_REGISTRY_PASSWORD" | docker login $(REGISTRY) -u "$$NIRVATI_REGISTRY_USERNAME" --password-stdin; \
	fi
